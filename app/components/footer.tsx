...
const socials = [
  ...
  {
    label: "Email",
    href: "mailto:hei.ryan.3@gmail.com",
    icon: (
...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-background border-t border-foreground/6 font-outfit">
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 py-12">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">

          {/* Logo + copyright */}
          <div className="flex flex-col gap-2">
            <span className="font-syne font-bold text-[20px] text-foreground tracking-tight">
          ...
          <div className="flex items-center gap-3">
            {socials.map(({ label, href, icon }) => (
              <a
                ...
                aria-label={label}
                className="flex items-center justify-center w-9 h-9 rounded-lg border border-foreground/10 text-muted hover:text-accent hover:border-accent/40 transition-all duration-200"
              >
                {icon}
              </a>
            ))}
